 ...
 * {@code GameScreen}, one per local player.
 */
public final class CharacterMeterRenderer {
    ...
    public static void draw(ShapeRenderer shapes, SpriteBatch sprites, BitmapFont font,
                             int characterId, float fill, float max, Color slotColor,
        ...
        if (def != null) {
            ...
            layout.setText(font, def.name);
            sprites.begin();
            font.setColor(ready ? Color.YELLOW : Color.WHITE);
            font.draw(sprites, def.name, x + (boxSize - layout.width) * 0.5f, y + boxSize * 0.08f + layout.height);
            sprites.end();
